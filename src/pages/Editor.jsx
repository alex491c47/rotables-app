                  ...
                  return (
                    <div className="tl-inline">
                      ...
                      {fhas("to") && <label>{eDef.cat === "out" ? "Customer city" : "Location"}
                        <select className="select" defaultValue={e.to || ""}
                          onChange={(ev2) => onEditEvent(idx, { to: ev2.target.value })}>
                          <option value="">— select —</option>
                          {CITY_NAMES.map((c) => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </label>}
    ...
    <div className="section">
      ...
      <div className="grid3">
        <Field label="Engine type">
          <select className="select" value={asset.aircraftType} onChange={(e) => set("aircraftType", e.target.value)}>
